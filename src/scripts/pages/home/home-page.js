...
export default class HomePage {
  ...
  async afterRender() {
    ...
    const map = L.map("map", { layers: [openStreetMap] }).setView(
      CONFIG.MAP_INITIAL_COORDS,
      CONFIG.MAP_INITIAL_ZOOM
    );
    L.control
      .layers({
        "Street View": openStreetMap,
        "Satelit View": esriWorldImagery,
      })
      .addTo(map);
    ...
    try {
      storyListContainer.innerHTML =
        '<p class="loading-message">Memuat data cerita...</p>';
      ...
      if (stories.length === 0) {
        storyListContainer.innerHTML =
          "<p>Belum ada cerita dengan lokasi yang dibagikan.</p>";
        return;
      }
      ...
      stories.forEach((story) => {
        ...
        storyListContainer.innerHTML += `
          <article class="story-item" data-id="${
            story.id
          }" tabindex="0" role="button" aria-label="Cerita oleh ${story.name}">
            <img class="story-item__image" src="${
              story.photoUrl
            }" alt="Foto cerita dari ${story.name}" crossorigin="anonymous">
            ...
            <button class="favorite-button" data-id="${story.id}" aria-label="${
